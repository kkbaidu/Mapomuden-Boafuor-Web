...
export const patientAPI = {
  ...
  getPatients: async (
    ...
    try {
      ...
      // Combine all patients and remove null/undefined + dedupe by _id
      const combined = [...patientsFromPopulation, ...additionalPatients];
      ...
      const enrichedPatients = uniquePatients
        .map((patient) => {
      ...
      const paginatedPatients = enrichedPatients.slice(offset, offset + limit);
  ...
  searchPatients: async (
    ...
    try {
      ...
      const filteredPatients = allPatientsResponse.data.filter(
        (patient) =>
